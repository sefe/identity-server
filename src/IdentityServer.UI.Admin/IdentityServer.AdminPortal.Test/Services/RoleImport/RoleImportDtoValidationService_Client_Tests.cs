...
[TestFixture]
public class RoleImportDtoValidationService_Client_Tests
{
    private const string _role1Name = "TestRole1";
    private const string _role2Name = "TestRole2";
    private static ClientRoleImportDto MakeImportDto(List<RoleValueObject<ClientRoleMappingValueObject>> roles) => new() { Roles = roles, ImportStrategy = ImportStrategy.Add };

    private static RoleValueObject<ClientRoleMappingValueObject> MakeRole(string name, params ClientRoleMappingValueObject[] mappings) => new()
    { RoleName = name, Mappings = new List<ClientRoleMappingValueObject>(mappings) };

    private static ClientRoleMappingValueObject MakeMapping(string type, string value, string desc) => new()
    { MappingType = type, Value = value, Description = desc };

    // Helper to match the new Validate signature
    private static bool Validate(ClientRoleImportDto dto, out string error)
    {
        var status = new OperationStatus();
        var result = RoleImportDtoValidationService<ClientRoleMappingValueObject>.IsValid(dto, status);
    ...
    {
        var dto = MakeImportDto(new List<RoleValueObject<ClientRoleMappingValueObject>>());
    ...
    {
        var dto = MakeImportDto(new List<RoleValueObject<ClientRoleMappingValueObject>> { new() { RoleName = null!, Mappings = new List<ClientRoleMappingValueObject>() } });
        var result = Validate(dto, out var error);
        using (Assert.EnterMultipleScope())
        {
            Assert.That(result, Is.False);
    ...
    {
        var dto = MakeImportDto(new List<RoleValueObject<ClientRoleMappingValueObject>> {
            MakeRole(_role1Name, new ClientRoleMappingValueObject { MappingType = null!, Value = "v", Description = "d" })
        });
        var result = Validate(dto, out var error);
        using (Assert.EnterMultipleScope())
        {
    ...
    {
        var dto = MakeImportDto(new List<RoleValueObject<ClientRoleMappingValueObject>> {
            MakeRole(_role1Name, new ClientRoleMappingValueObject { MappingType = nameof(RoleMapType.SecurityGroup), Value = null!, Description = "d" })
        });
        var result = Validate(dto, out var error);
        using (Assert.EnterMultipleScope())
        {
    ...
    {
        var dto = MakeImportDto(new List<RoleValueObject<ClientRoleMappingValueObject>> {
            MakeRole(_role1Name, new ClientRoleMappingValueObject { MappingType = nameof(RoleMapType.SecurityGroup), Value = "v", Description = null! })
        });
        var result = Validate(dto, out var error);
        using (Assert.EnterMultipleScope())
        {

...
[TestFixture]
public class RoleReplaceComparerTests
{
    ...
    {
        var result = await RoleReplaceComparer.CompareRolesAsync(new List<RoleValueObject<ApiResourceRoleMappingValueObject>>(), new List<RoleValueObject<ApiResourceRoleMappingValueObject>>());
        Assert.That(result, Is.Empty);
    }

    [Test]
    public async Task CompareRoles_RoleOnlyInExisting_ReturnsRemoved()
    {
        ...
        var result = await RoleReplaceComparer.CompareRolesAsync(existing, imported);
        Assert.That(result, Has.Count.EqualTo(1));
        using (Assert.EnterMultipleScope())
        {
            Assert.That(result[0].RoleName, Is.EqualTo(_role1Name));
            Assert.That(result[0].RoleState, Is.EqualTo(ComparisonState.Removed));
        }
    }
    ...
    [Test]
    public async Task CompareRoles_RoleInBothWithChangedMappingDescription_ReturnsChanged()
    {
        var existing = new List<RoleValueObject<ApiResourceRoleMappingValueObject>>
        {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(role.RoleName, Is.EqualTo(_role1Name));
            Assert.That(role.RoleState, Is.EqualTo(ComparisonState.Unchanged));
            Assert.That(role.RoleMappingState, Is.EqualTo(ComparisonState.Changed));
        }
        var mappings = role.Mappings;
        Assert.That(mappings, Has.Count.EqualTo(1));
        Assert.That(mappings[0].State, Is.EqualTo(ComparisonState.Changed));
    }

    [Test]
    public async Task CompareRoles_RoleInBothWithAddedAndRemovedMappings()
    {
        var existing = new List<RoleValueObject<ApiResourceRoleMappingValueObject>>
        {
            MakeRole(_role1Name,
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(mappings.Any(m => m.State == ComparisonState.Removed && m.Existing.MappingType == RoleMapTypeStrings.ClientId), Is.True);
    ...
    public void CompareRoles_RoleWithNullMappings_DoesNotThrow()
    {
        ...
        Assert.DoesNotThrowAsync(() => RoleReplaceComparer.CompareRolesAsync(existing, imported));
    }
}

...
[TestFixture]
public class ApiResourceRoleMapPickerTests
{
    private TestContext _ctx;
    private ISearchProvider<Group> _mockGroupSearchProvider;
    private ISearchProvider<ClientShortDtoRead> _mockClientSearchProvider;
    private ISearchProvider<User> _mockUserSearchProvider;
    private List<ApiResourcePropertyRoleDtoRead> _testRoles;
    ...
    public void Setup()
    {
        ...
        _mockClientSearchProvider = Substitute.For<ISearchProvider<ClientShortDtoRead>>();
        ...
        _ctx.Services.AddSingleton(_mockClientSearchProvider);
    ...
    {
        // Arrange & Act
        var cut = _ctx.RenderComponent<ApiResourceRoleMapPicker>(parameters => parameters
    ...
    [TestCase(RoleMapType.SecurityGroup, "Security Group")]
    [TestCase(RoleMapType.UserObjectId, "User")]
    [TestCase(RoleMapType.ClientId, "Client")]
    public async Task OpenRoleMappingPicker_RendersOffcanvasWithCorrectTitle(RoleMapType roleMapType, string friendlyName)
    {
        // Arrange
        var roleMapping = new ApiResourceRoleMappingDtoCreateBuilder(roleMapType).Build();
        var cut = _ctx.RenderComponent<ApiResourceRoleMapPicker>(parameters => parameters
            .Add(p => p.ExistingRoles, _testRoles));
        ...
        // Assert
        var title = cut.Find("#apiRoleMapEditorTitle");
    ...
    {
        // Arrange
        var roleMapping = new ApiResourceRoleMappingDtoCreateBuilder(RoleMapType.SecurityGroup).Build();
        var cut = _ctx.RenderComponent<ApiResourceRoleMapPicker>(parameters => parameters
            .Add(p => p.ExistingRoles, _testRoles));
    ...
    [Test]
    public async Task SetSelectedRole_WithClientIdMapping_ShowsClientSearchPickList()
    {
        // Arrange
        var roleMapping = new ApiResourceRoleMappingDtoCreateBuilder(RoleMapType.ClientId).Build();
        SetupClientSearchResponse();
        var cut = _ctx.RenderComponent<ApiResourceRoleMapPicker>(parameters => parameters
            .Add(p => p.ExistingRoles, _testRoles));
        await cut.InvokeAsync(() => cut.Instance.OpenRoleMappingPicker(roleMapping));
        ...
        // Assert
        var searchPickList = cut.FindComponent<SearchPickList<int, ClientShortDtoRead>>();
        Assert.That(searchPickList, Is.Not.Null);
    }
    ...
    {
        // Arrange
        var roleMapping = new ApiResourceRoleMappingDtoCreateBuilder(RoleMapType.UserObjectId).Build();
        var cut = _ctx.RenderComponent<ApiResourceRoleMapPicker>(parameters => parameters
            .Add(p => p.ExistingRoles, _testRoles));
        await cut.InvokeAsync(() => cut.Instance.OpenRoleMappingPicker(roleMapping));
    ...
    public async Task SetSelectedEntraGroup_WhenClicked_InvokesOnRoleMappingConfirmedCallbackAndClosesDrawer()
    {
        ...
        ApiResourcePropertyRoleMappingDtoCreate capturedRoleMapping = null;

        var roleMapping = new ApiResourceRoleMappingDtoCreateBuilder(RoleMapType.SecurityGroup).Build();

        var cut = _ctx.RenderComponent<ApiResourceRoleMapPicker>(parameters => parameters
            .Add(p => p.ExistingRoles, _testRoles)
            .Add(p => p.OnRoleMappingConfirmed, EventCallback.Factory.Create<ApiResourcePropertyRoleMappingDtoCreate>(this, mapping =>
            {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(capturedRoleMapping.ApiResourceRoleId, Is.EqualTo(1));
            Assert.That(capturedRoleMapping.MappingType, Is.EqualTo(RoleMapType.SecurityGroup));
            Assert.That(cut.FindAll(".offcanvas"), Has.Count.EqualTo(0));
        }
    }

    [Test]
    public async Task SetSelectedClient_WhenClicked_InvokesOnRoleMappingConfirmedCallbackAndClosesDrawer()
    {
        // Arrange
        var callbackInvoked = false;
        ApiResourcePropertyRoleMappingDtoCreate capturedRoleMapping = null;
        ...
        await cut.InvokeAsync(() => cut.Instance.OpenRoleMappingPicker(roleMapping));
        await SelectFirstVisibleRoleButtonAsync(cut);
        var testClient = new ClientShortDtoRead { ClientId = "test-client-id", ClientName = "Test Client" };

        // Act
        await cut.InvokeAsync(() => cut.Instance.SetSelectedClient(testClient));
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(callbackInvoked, Is.True);
            Assert.That(capturedRoleMapping, Is.Not.Null);
            Assert.That(capturedRoleMapping.Value, Is.EqualTo("test-client-id"));
            Assert.That(capturedRoleMapping.ApiResourceRoleId, Is.EqualTo(1));
            Assert.That(capturedRoleMapping.MappingType, Is.EqualTo(RoleMapType.ClientId));
            Assert.That(cut.FindAll(".offcanvas"), Has.Count.EqualTo(0));
        }
    }
    ...
    public async Task SetSelectedUser_WhenClicked_InvokesOnRoleMappingConfirmedCallbackAndClosesDrawer()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(capturedRoleMapping.ApiResourceRoleId, Is.EqualTo(1));
            Assert.That(capturedRoleMapping.MappingType, Is.EqualTo(RoleMapType.UserObjectId));
            Assert.That(cut.FindAll(".offcanvas"), Has.Count.EqualTo(0));
        }
    }
    ...
    [Test]
    public async Task SetSelectedRole_WithExistingMappings_ExcludesAlreadyAssignedClients()
    {
        // Arrange
        var rolesWithMappings = CreateTestRolesWithMappings();
        var roleMapping = new ApiResourceRoleMappingDtoCreateBuilder(RoleMapType.ClientId).Build();
        ...
        {
            Assert.That(excludeItems, Has.Count.EqualTo(1));
            Assert.That(excludeItems.First().ClientId, Is.EqualTo("existing-client-id"));
        }
    }
    ...
    private static async Task SelectFirstVisibleRoleButtonAsync(IRenderedComponent<ApiResourceRoleMapPicker> cut)
    {
    ...
    private static List<ApiResourcePropertyRoleDtoRead> CreateTestRoles()
    {
        return new List<ApiResourcePropertyRoleDtoRead>
        {
            new()
            {
                Id = 1,
    ...
    private static List<ApiResourcePropertyRoleDtoRead> CreateTestRolesWithMappings()
    {
        return new List<ApiResourcePropertyRoleDtoRead>
        {
            new()
    ...
    private void SetupClientSearchResponse()
    {
        _mockClientSearchProvider.SearchAsync(Arg.Any<string>(), Arg.Any<string>()).Returns(new SearchResult2<ClientShortDtoRead>() { Page = new List<ClientShortDtoRead>() { new() { ClientId = "1", ClientName = "Client1" } } });
    }
    ...
    private void SetupTelerikInterop()
    {
        ...
        _ctx.JSInterop.SetupVoid("TelerikBlazor.initComponentLoaderContainer", _ => true);
        _ctx.JSInterop.SetupVoid("TelerikBlazor.initListView", _ => true);

...
using IdentityServer.AdminPortal.Web.Pages.ApiResource;
using IdentityServer.AdminPortal.Web.Services;
using IdentityServer.Tests.Common;
using TestContext = Bunit.TestContext;

namespace IdentityServer.AdminPortal.Web.Tests.Pages.ApiResource;

[TestFixture]
public class ApiResourceRoleImportPageTests
{
    ...
    [Test]
    public void Render_WithNoApiResource_ShowsLoadingMessage()
    {
        // Arrange
        // Don't setup any HTTP response, which will cause ApiResource to remain null

        // Act
        var cut = _ctx.RenderComponent<ApiResourceRoleImportPage>(parameters => parameters
            .Add(p => p.ApiResourceId, 1));
    ...
    [Test]
    public void Render_WithValidApiResource_ShowsApiResourceNameAndImportWizard()
    {
        // Arrange
        var testApiResource = CreateTestApiResource();
        SetupGetApiResourceResponse(HttpStatusCode.OK, testApiResource);

        // Act
        var cut = _ctx.RenderComponent<ApiResourceRoleImportPage>(parameters => parameters
            .Add(p => p.ApiResourceId, testApiResource.Id));

        // Assert
        using (Assert.EnterMultipleScope())
        {
            Assert.That(cut.Markup, Does.Not.Contain("<p><em>Loading...</em></p>"));
            Assert.That(cut.Markup, Does.Contain($"Import Role Mappings for API Resource '{testApiResource.DisplayName}'"));
            Assert.That(cut.Markup, Does.Contain("Back to the API Resource"));
        }
    }

    [Test]
    public async Task ValidateImportApiResourceRoles_WithSuccessfulValidation_UpdatesImportModelWithSuccessStatus()
    {
        // Arrange
        var testApiResource = CreateTestApiResource();
        ...
        SetupMultipleHttpResponses(testApiResource, successOperationStatus);

        var cut = _ctx.RenderComponent<ApiResourceRoleImportPage>(parameters => parameters
            .Add(p => p.ApiResourceId, testApiResource.Id));

        // Wait for the component to load the API resource
        await cut.InvokeAsync(async () => await Task.Delay(100));

        // Act
        await cut.InvokeAsync(async () => await cut.Instance.ValidateImportApiResourceRoles(importModel));
    ...
    [Test]
    public async Task ValidateImportApiResourceRoles_WithValidationErrors_UpdatesImportModelWithErrors()
    {
        // Arrange
        var testApiResource = CreateTestApiResource();
        ...
        SetupMultipleHttpResponses(testApiResource, errorOperationStatus);
    ...
    [Test]
    public async Task ValidateImportApiResourceRoles_WithApiCallFailureAndFlatErrors_UpdatesImportModelWithErrors()
    {
        // Arrange
        var testApiResource = CreateTestApiResource();
        ...
        SetupMultipleHttpResponses(testApiResource, errorOperationStatus, HttpStatusCode.BadRequest);
    ...
    [Test]
    public async Task ValidateImportApiResourceRoles_WithApiCallFailureAndNoErrors_UpdatesImportModelWithErrors()
    {
        // Arrange
        var testApiResource = CreateTestApiResource();
        var errorOperationStatus = new ProblemDetails()
    ...
    private void SetupCommonServices()
    {
        ...
        SetupGetApiResourceResponse(HttpStatusCode.NotFound, null);
    ...
    private void SetupGetApiResourceResponse(HttpStatusCode statusCode, ApiResourceDtoRead apiResource)
    {
        var json = apiResource == null ? "{}" : JsonSerializer.Serialize(apiResource);
    ...
    private void SetupMultipleHttpResponses(ApiResourceDtoRead apiResource, object importValidationResult = null, HttpStatusCode httpStatusCode = HttpStatusCode.OK)
    {
        var responses = new List<(HttpStatusCode StatusCode, object Response)>
        {
            (HttpStatusCode.OK, apiResource), // First response: GetApiResource
            (httpStatusCode, importValidationResult) // Second response: ValidateImportApiResourceRoles
        };
    ...
    private static ApiResourceRoleImportModel CreateTestImportModel()
    {
        ...
        return new ApiResourceRoleImportModel
        {
            FileParsingStatus = new OperationStatus<ApiResourceRoleImportDto> { Result = importDto, IsCompleted = true },

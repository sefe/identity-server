...
public class ApplicationListPageTests
{
    ...
    [Test]
    public void ApplicationListPage_WithGivenParameters_RendersGrid()
    {
        // Arrange
        SetupAuthentication(false);

        var applications = new List<ClientShortDtoRead>
        {
            new() { Id = 1, ClientId = "app1", ClientName = "Application One", SystemPermissionId = 1, SystemPermissionName = "Test Permission", SystemPermissionEnvironmentId = 1, SystemPermissionEnvironmentName = "Dev", SystemPermissionEnvironmentOwnersList = new() {"Owner1" } }
        };
        var dataEnvelope = new DataEnvelope<ClientShortDtoRead>() { CurrentPageData = applications, TotalItemCount = applications.Count };
        var apiCallResult = new ApiCallResult<DataEnvelope<ClientShortDtoRead>>(dataEnvelope);

        _mockAdminApiService
            .GetClientsPaged(Arg.Any<DataSourceRequest>())
            .Returns(Task.FromResult(apiCallResult));

        // Act
        var cut = _ctx.RenderComponent<TelerikRootComponent>(parameters => parameters
            .AddChildContent<ApplicationListPage>()
        );

        // Assert
        using (Assert.EnterMultipleScope())
        {
            Assert.That(cut.Markup, Does.Contain("Show only My Applications"));
            Assert.That(cut.Markup, Does.Contain("app1"));
        }
    }

    [Test]
    public void ApplicationListPage_WithAdmin_DoesnotRenderMyResourcesSwitch()
    {
        // Arrange
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(cut.Markup, Does.Not.Contain("Show only My Applications"));
        }
    }

    [Test]
    public async Task ApplicationListPage_OnShowMyItemsToggleChanged_AppliesAccessLevelFilter()
    {
        // Arrange
        ...
        var applications = new List<ClientShortDtoRead>
        {
            new() { Id = 1, ClientId = "app1", ClientName = "Application One", AccessLevel = SystemPermissionRoleType.None, SystemPermissionId = 1, SystemPermissionName = "Test Permission", SystemPermissionEnvironmentId = 1, SystemPermissionEnvironmentName = "Dev", SystemPermissionEnvironmentOwnersList = new() {"Owner1" } }
        };
        var dataEnvelope = new DataEnvelope<ClientShortDtoRead>() { CurrentPageData = applications, TotalItemCount = applications.Count };
        var apiCallResult = new ApiCallResult<DataEnvelope<ClientShortDtoRead>>(dataEnvelope);
